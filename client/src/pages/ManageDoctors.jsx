...
import { useAllDoctors, useCreateDoctor, useUpdateDoctor, useDeactivateDoctor } from "../hooks/useAdmin";
...
const ManageDoctors = () => {
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctors.filter((doctor) => {
      ...
      return [name, email, specialization, department].some((value) => value.toLowerCase().includes(needle));
    });
  }, [doctors, query]);
  ...
  const handleDeactivate = (doctor) => {
    if (window.confirm(`Deactivate Dr. ${doctor.userId?.name || "this doctor"}?`)) {
      deactivateMutation.mutate(doctor._id);
    }
  };

  if (isLoading) {
    return <div className="text-center py-20 text-gray-400">Loading doctors...</div>;
  }

  if (isError) {
    return <div className="text-center py-20 text-red-400">Failed to load doctors.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-wide">Manage Doctors</h1>
          <p className="text-sm text-gray-400 mt-1">Create, edit, and deactivate doctor accounts</p>
        </div>
        <button
          onClick={openCreate}
                  ...
                  return (
                    <tr key={doctor._id} className="hover:bg-white/5 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-medium text-white">{doctor.userId?.name || "N/A"}</p>
                        <p className="text-xs text-gray-500">{doctor.userId?.email || "N/A"}</p>
                      </td>
                      <td className="px-6 py-4 text-emerald-400">{doctor.specialization || "N/A"}</td>
                      <td className="px-6 py-4 text-gray-300">{doctor.department || "N/A"}</td>
                      <td className="px-6 py-4 text-gray-400">{doctor.availableDays?.length ? doctor.availableDays.join(", ") : "N/A"}</td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 py-6">
          <div className="w-full max-w-2xl bg-[#1f2937] border border-gray-700 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between bg-[#111827]">
              <div>
                <h2 className="text-lg font-semibold text-white">{editingDoctor ? "Edit Doctor" : "Create Doctor"}</h2>
                <p className="text-xs text-gray-400">{editingDoctor ? "Update account and profile details" : "Create a new doctor account and profile"}</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {DAYS.map((day) => {
                    ...
                    return (
                      <label key={day} className={`cursor-pointer rounded-xl border px-3 py-2 text-sm transition-colors ${checked ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-300" : "bg-[#111827] border-gray-700 text-gray-300"}`}>
                        <input
                          type="checkbox"
                          checked={checked}
